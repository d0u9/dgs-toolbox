...
| `box.trash.keep` | How many days the trash is worth keeping, shown as advice in `dgs box view`. `box` never removes a file: emptying `trash/` is done by hand. `0` shows no advice. | `90` |
