...
when that variable is unset). That last one is the only default location.
`--config` deliberately bypasses the environment variable, so a run can point
at a whole configuration without changing the shell it was launched from.
