...
| `doc.targets` | Named local folders a View is exported to, `{"name": "folder"}`. The page offers them by name, as shortcuts beside any folder chosen in its file dialog. An export writes only there, and removes only files its own manifest, `dgs-export.json`, says it wrote. A leading `~` is the home directory. A Target may not be inside the tree, nor the tree inside it. | empty — no Targets |
