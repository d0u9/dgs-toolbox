...
4. Read the temporary file sequentially and compute the Destination SHA-256 digest. This second destination read is required by the integrity contract and is not considered redundant scanning. An error closing this read-only handle after a complete read is not a failure: the digest is already of every byte, and nothing is pending on the handle. macOS SMB mounts report `EBADF` there.
