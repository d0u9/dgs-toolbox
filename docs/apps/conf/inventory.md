...
| Manifest keys | `secret`, `roles`, and a role's `auth`, `reached_by`, `template`, `defaults`, `output`, `rotation`, `own`, `combine_own` |
| Enumerated values | `devices: unmanaged`; `auth: per-principal`, `none`; `defaults: document`, `element`; `client_role: none`; `rotation: disruptive` |
...
still produces a grant, but no file for the person's device. The address and
the password reach them some other way, which is the truth of how such a
service is used and not something to invent a file for.
...
| `auth` | What a grant on one of this role's ports implies |
| --- | --- |
| `per-principal` | One secret per principal reaching each port, and a rendered account table |
| `none` | Nothing |

There is no third value. An earlier `shared` meant "one own secret for the
instance, embedded in every client that reaches it", and that turned out to be
two separate things: [`own`](#a-roles-own-secrets) declares the secret, and
[`combine_own`](#a-shared-identity-alongside-a-principals-own) says it travels
to the clients. Either can be used without the other — MicroBin's passwords go
to no client, and a role could combine a secret it shares for a reason of its
own — so `auth` keeps only the question it was always answering: does a
principal reaching this role get a credential of its own. A manifest still
declaring `shared` is a broken manifest, named with its role, rather than a
role that silently generates nothing.
...
### A role's own secrets

`auth` describes the inbound side: what a principal needs to reach this role.
A role also has credentials of its own, belonging to the instance rather than
to anything reaching it — an administrative password, an upload password, a
TLS private key. They are independent of `auth`, and a role declares them by
name:
...
roles:
  server:
    auth: none
    own: [auth_password, admin_password, upload_password]
```

Each name is one `<instance>/own/<name>` file, one per instance of the role,
generated by `secret sync` in the shape the service's `secret` block declares.
MicroBin authenticates nobody in the inventory's sense — nothing routes
through it, no principal holds a grant on it — and still holds three
passwords, which is why the two are separate keys.

**The list is the only place a role's own secrets are named.** That is what
makes the tree computable in both directions: a name in the list with no file
is generated, and a file under `own/` no list accounts for is reported
orphaned like any other. Without it, the two are the same fact on disk, and
`sync` can only leave both alone.
...
you see it, on every machine, forever. It is configuration, and belongs in the
role's `defaults.yaml` where a change to it is a diff someone can read. The
...
`combine_own` names one of [the role's own secrets](#a-roles-own-secrets) —
`<instance>/own/psk` — and says that every client deriving from this role needs
it too. It names, and does not declare: `psk` is generated because it is in the
role's `own` list, and `combine_own` only marks which of those names travels to
the clients, as `upstream.own` beside `upstream.secret` for the client's own
principal value; see [the render context](#the-render-context). A
`combine_own` naming something outside the `own` list is an error, since it
would name a file nothing generates.

A role with no `combine_own` gives clients nothing beyond their own secret,
which is the ordinary case.

### Keeping the tree in step

`dgs conf secret sync` compares the paths the inventory implies against the
files on disk. Two things imply a path: a `per-principal` role implies one per
(principal, port) grant, and a role's [`own` list](#a-roles-own-secrets)
implies one per name per instance. Missing paths are generated. Paths on disk
...
2. Every `instance.service` names a service, and every `instance.role` names one
   of its roles. A `reached_by` names another role of the same service, and a
